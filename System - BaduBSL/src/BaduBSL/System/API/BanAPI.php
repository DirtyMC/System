...
class BanAPI {
    ...
    public function isBanned(string $player): bool {
        $stmt = $this->db->prepare("SELECT player FROM bans WHERE player = ? AND (expires IS NULL OR expires > ?)");
        $now = time();
    ...
    public function banPlayer(string $player, string $reason, ?int $duration = null): void {
        $expires = $duration !== null ? time() + $duration : null;

        $stmt = $this->db->prepare("REPLACE INTO bans (player, reason, expires) VALUES (?, ?, ?)");
    ...
    public function unbanPlayer(string $player): void {
        $stmt = $this->db->prepare("DELETE FROM bans WHERE player = ?");
    ...
    public function getBanReason(string $player): string {
        $stmt = $this->db->prepare("SELECT reason FROM bans WHERE player = ?");
        $stmt->bind_param("s", $player);
        $stmt->execute();
    ...
    public function cleanExpiredBans(): void {
        $now = time();
        $stmt = $this->db->prepare("DELETE FROM bans WHERE expires <= ?");
